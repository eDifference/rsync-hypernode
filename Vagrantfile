...
if settings['settings'].nil? or settings['settings']['magento_ver'].nil?
	settings_magento_version = 2
else
	settings_magento_version = settings['settings']['magento_ver']
end
...
# abort if vagrant-auto_networking is not installed
if !Vagrant.has_plugin?("vagrant-auto_network")
        abort "Please install the 'vagrant-auto_network' module"
end

AutoNetwork.default_pool = "10.2.35.0/24"
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell" do |s|
    s.path = "vagrant/provisioning/hypernode.sh"
    s.args = "#{settings_magento_version}"
  end
  ...
  if Vagrant.has_plugin?("vagrant-hostmanager")
    ...
    config.vm.define 'hypernode' do |node|
      ...
      node.hostmanager.aliases = ["hypernode.local", directory_alias, directory_hash_alias]
